...
class KMeans() {
  ...
  @tailrec
  private def kmeans(species: ParVector[Specy], means: ParArray[Specy], eta: Double): ParArray[Specy] = {
    val newMeans =
      means.map(mean => {
        species.groupBy(p => means.minBy(_.distance(p)))
               .get(mean) match {
                    case Some(c) => c.sum / c.length
                    case None => mean
                }
      })
    val converged = (means zip newMeans).forall {
      case (oldMean, newMean) => oldMean.distance(newMean) <= eta
    }
    if (!converged) kmeans(species, newMeans, eta) else newMeans
  }
}
...
object KMeans {
  def main(args: Array[String]): Unit = {
    ...
    kmeans.kmeans(parPoints, parMeans, eta).foreach {
      println
    }
  }

  def apply(): KMeans = new KMeans()
}